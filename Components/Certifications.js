    ...
    <section className="max-w-5xl mx-auto px-4 mb-16 w-full">
      ...
      <div className="flex flex-col gap-3">
        {certs.map((cert) => (
          <div
            key={cert.abbr}
            className={`flex items-center gap-4 bg-white rounded-xl px-4 py-3.5 border transition-colors duration-200 dark:bg-neutral-900 ${
          ...
          >
            {/* Abbreviation tile */}
            <div
              className={`flex-shrink-0 w-12 h-12 rounded-lg flex items-center justify-center text-xs font-black tracking-tight ${
                cert.status === 'certified'
            ...
            <div className="min-w-0 flex-1">
              <p className="text-sm font-semibold text-neutral-900 dark:text-white">{cert.name}</p>
              <p className="text-xs text-neutral-400 dark:text-neutral-500 mt-0.5">{cert.issuer}</p>
            </div>
